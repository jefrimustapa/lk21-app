                    ...
                    if ("GET".equalsIgnoreCase(method) && (url.contains("video.php") || url.contains("/iframe/"))) {
                        try {
                            ...
                            String injectScript = "<script>\n"
                                 ...
                                 + "  try {\n"
                                 + "    var now = Date.now();\n"
                                 + "    if (now - __lastToggleTime < 400) return;\n"
                                 ...
                                 + "  try {\n"
                                 + "    if (window.p2p && p2p.player && typeof p2p.player.seek === 'function') {\n"
                                 + "      p2p.player.seek(targetSeconds);\n"
                                 + "      return;\n"
                                 + "    }\n"
                                 + "    if (window.jwplayer && typeof window.jwplayer === 'function') {\n"
                                 + "      var jw = window.jwplayer();\n"
                                 + "      if (jw && typeof jw.seek === 'function') {\n"
                                 + "        jw.seek(targetSeconds);\n"
                                 + "        return;\n"
                                 + "      }\n"
                                 + "    }\n"
                                 + "    var v = document.querySelector('video');\n"
                                 ...
                                 + "        postToParent({ type: 'pause', state: 'paused' });\n"
                                 + "      });\n"
                                 + "      p2p.player.on('time', function(t) {\n"
                                 + "        try {\n"
                                 + "          postToParent({\n"
                                 + "            type: 'timeUpdate',\n"
                                 + "            currentTime: p2p.player.getPosition ? p2p.player.getPosition() : (t ? t.position : 0),\n"
                                 + "            duration: p2p.player.getDuration ? p2p.player.getDuration() : (t ? t.duration : 0)\n"
                                 + "          });\n"
                                 + "        } catch(err){}\n"
                                 + "      });\n"
                                 + "    }\n"
                                 ...
                                 + "      if (v && v.src && v.src.indexOf('http') === 0 && !v.src.includes('blob:')) {\n"
                                 + "        detectedUrl = v.src;\n"
                                 + "      } else if (v && v.currentSrc && v.currentSrc.indexOf('http') === 0 && !v.currentSrc.includes('blob:')) {\n"
                                 + "        detectedUrl = v.currentSrc;\n"
                                 + "      }\n"
                                 + "      if (!detectedUrl && window.jwplayer && typeof window.jwplayer === 'function') {\n"
                                 + "        var jw2 = window.jwplayer();\n"
                                 + "        var item = jw2 ? (jw2.getPlaylistItem ? jw2.getPlaylistItem() : (jw2.getPlaylist ? jw2.getPlaylist()[0] : null)) : null;\n"
                                 + "        if (item && item.file && item.file.indexOf('http') === 0) {\n"
                                 + "          detectedUrl = item.file;\n"
                                 + "        } else if (item && item.sources && item.sources.length > 0 && item.sources[0].file) {\n"
                                 + "          detectedUrl = item.sources[0].file;\n"
                                 + "        }\n"
                                 + "      }\n"
                                 + "      if (!detectedUrl && window.p2p && p2p.player && p2p.player.url && p2p.player.url.indexOf('http') === 0) {\n"
                                 + "        detectedUrl = p2p.player.url;\n"
                                 + "      }\n"
                                 + "      if (detectedUrl && !window.__streamDetached) {\n"
                                 + "        window.__streamDetached = true;\n"
                                 + "        postToParent({\n"
                                 + "          type: 'streamDetected',\n"
                                 ...
                                 + "  } catch(e) {}\n"
                                 + "}\n"
                                 + "setInterval(setupPlaybackDetection, 1000);\n"
                                 ...
                                 + "  } else if (data.type === 'seekTo' && typeof data.position === 'number') {\n"
                                 + "    handleSeekToPosition(data.position);\n"
                                 + "  } else if (data.type === 'togglePlay' || data.func === 'togglePlay' || data === 'togglePlay') {\n"
                                 + "    triggerToggle();\n"
                                 + "  }\n"
                                 + "});\n"
                                 + "document.addEventListener('touchstart', function() {\n"
                                 + "  postToParent({ type: 'userActivity' });\n"
                                 + "}, { passive: true });\n"
