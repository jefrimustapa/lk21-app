export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Endpoint 2: Paginated movies list
      if (path === "/api/movies") {
        ...
        const total = countStmt ? countStmt.count : 0;

        const { results } = await env.DB.prepare("SELECT * FROM movies ORDER BY id DESC LIMIT ? OFFSET ?")
          .bind(limit, offset)
          .all();
      ...
      if (path === "/api/search") {
        ...
        const { results } = await env.DB.prepare("SELECT * FROM movies WHERE title LIKE ? OR synopsis LIKE ? LIMIT 50")
          .bind(`%${query}%`, `%${query}%`)
          .all();
